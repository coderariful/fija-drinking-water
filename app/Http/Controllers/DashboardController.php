...
class DashboardController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $total_sell_today = Transaction::commonQuery()->today()->sum('total_amount');
        $total_collect_today =  Transaction::commonQuery()->today()->sum('paid_amount');
        $due_today = $total_sell_today - $total_collect_today;

        $jar_in_qty = Transaction::commonQuery()->where('product_type', Product::WATER)->sum('in_quantity');
        $jar_out_qty = Transaction::commonQuery()->where('product_type', Product::WATER)->sum('out_quantity');

        $total_due = Transaction::commonQuery()->sum('total_amount') - Transaction::commonQuery()->sum('paid_amount');
        ...
        return view('admin.dashboard', [
            ...
            'jar_sale_today' => Transaction::commonQuery()->today()->where('product_type', Product::WATER)->count(),
            'jar_sale_this_month' => Transaction::commonQuery()->thisMonth()->where('product_type', Product::WATER)->count(),
    ...
    public function userDashboard()
    {
        ...
        $jar_stock = Transaction::commonQuery()->whereRelation('customer', 'user_id', $user_id)->where('product_type', Product::WATER)->selectRaw('SUM(in_quantity) as in_qty, SUM(out_quantity) as out_qty')->first();
        $total_sell_today = Transaction::commonQuery()->where('user_id', $user_id)->today()->sum('total_amount');
        $total_collect_today =  Transaction::commonQuery()->where('user_id', $user_id)->today()->sum('paid_amount');
        $due_today = $total_sell_today - $total_collect_today;

        $total_sell = Transaction::commonQuery()->where('user_id', $user_id)->sum('total_amount');
        $total_paid = Transaction::commonQuery()->where('user_id', $user_id)->sum('paid_amount');
        ...
        return view('user.dashboard', [
            ...
            'jar_sale_today' => Transaction::commonQuery()->today()->whereUserId($user_id)->where('product_type', Product::WATER)->count(),
            'jar_sale_this_month' => Transaction::commonQuery()->thisMonth()->whereUserId($user_id)->where('product_type', Product::WATER)->count(),
