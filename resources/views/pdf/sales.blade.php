...
<body>
    <div class="container-fluid">
        <img src="{{asset('logo.png')}}" alt="watermark"
            style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);opacity:0.1;z-index:-999;">
        <div class="mb-4">
            ...
            <div class="mt-2 text-center">
                <span class="mx-3">
                    <strong>{{__("Total Due")}}:</strong> {{$total_due}}
                </span>
            ...
            <div class="text-center mt-2">
                <strong>{{trans('Print Date')}}:</strong> {{today()->format(DATE_FORMAT)}}
            </div>
        </div>
        <div class="mb-3">
                ...
                <tr>
                    <td>
                        <div class="text-center">
                            <p class="h3 mb-0">{{($jar_in_count + $jar_in_previous) - ($jar_out_count + $jar_out_previous)}}
                            </p>
                    ...
                    <tr>
                        ...
                        @if(count($histories) > 1)
                            ...
                            <td class="text-left py-0" rowspan="{{count($histories) + 1}}">
                                {{$history?->customer?->name}}<br>{{$history?->customer?->address}}</td>
                            <td class="text-center py-0" rowspan="{{count($histories) + 1}}">
                                {{round($history?->customer?->jar_rate, 2)}}/-</td>
                        @else
                            <td class="text-center align-middle">{{$loop->iteration}}</td>
                            <td class="text-center py-0">{{$history?->customer?->phone}}</td>
                            <td class="text-left py-0">{{$history?->customer?->name}}<br>{{$history?->customer?->address}}
                            </td>
    ...
    {{--@isset($download_pdf)
        <a href="{{ $download_pdf }}" class="btn btn-info download-pdf-btn">Download PDF</a>
    @endisset--}}
</body>
