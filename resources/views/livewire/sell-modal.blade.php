<div x-data="dataSellModal">
    ...
    <div class="row">
        <div class="form-group col-4">
            ...
            <input id="rate" type="number" step="any" class="form-control" wire:model="rate" min="0"/>
            @error('rate')
            <span class="text-danger">{{$message}}</span>
            @enderror
        </div>
        <div class="form-group col-4">
            ...
            <input id="quantity" type="number" class="form-control" wire:model="quantity" min="0"/>
            @error('quantity')
            <span class="text-danger">{{$message}}</span>
            @enderror
        </div>
        <div class="form-group col-4">
            ...
            <input id="out_quantity" type="number" class="form-control" wire:model="out_quantity" min="0"/>
            @error('out_quantity')
            <span class="text-danger">{{$message}}</span>
            @enderror
        </div>
        <div class="form-group {{auth_user()->isAdmin()?'col-4':'col-6'}}">
            ...
            <input id="total_cost" type="number" step="any" class="form-control bg-white text-dark"
                   wire:model.defer="total_cost" min="0" readonly/>
            @error('total_cost')
            <span class="text-danger">{{$message}}</span>
            @enderror
        </div>
        ...
        @if(auth_user()->isAdmin())
            <div class="form-group col-4">
                ...
                <input id="date" type="date" class="form-control bg-white text-dark" wire:model.defer="date" min="0"/>
                @error('date')
                <span class="text-danger">{{$message}}</span>
                @enderror
            </div>
    ...
    <script>
        document.addEventListener('alpine:init', function () {
            Alpine.data('dataSellModal', () => ({
                ...
                total_cost: @entangle('total_cost'),
            }))
        })
    </script>
@endpush
