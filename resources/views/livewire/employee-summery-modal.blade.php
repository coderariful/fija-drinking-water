<div class="position-relative" x-data="{type:@entangle('filterType').live}">
    ...
    <div class="text-center">
        @if($user)
            @php($_params=['user'=>$user->id,'customer_id'=>$customer_id,'filter'=>$filterType,'date'=>[$start_date,$end_date],'month'=>$month,'year'=>$year,'day'=>$day,'view'=>'on'])
            <a href="{{route('print.sales-list', $_params)}}" class="btn btn-danger" target="_blank">{{trans("PRINT")}}</a>
        @endif
    </div>
</div>
